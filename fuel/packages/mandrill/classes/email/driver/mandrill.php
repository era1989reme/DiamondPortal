...
class Email_Driver_Mandrill extends Email_Driver {
	...
	}	if(!is_null($this->template_name) || !empty($this->template_name))  { 		$message->sendTemplate($this->template_name,[], $message_params,  \Config::get('mandrill.async'));	}else{ 		$message->send($message_params, \Config::get('mandrill.async'));	}
		//
    ...
    }		public function setTemplateName($template = null)	{ 		$this->template_name = $template;	}
